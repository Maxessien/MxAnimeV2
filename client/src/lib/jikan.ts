...
export const MAL_DEFAULT_FIELDS = "id,title,main_picture,alternative_titles,media_type,source,num_episodes,status,mean,num_scoring_users,rank,popularity,synopsis,start_season,studios,genres"
  ...
  return new Promise((resolve, reject) => {
    const task = async () => {
      try {
        ...
        // 🌟 FIX: Use the native Headers class explicitly
        const headers = new Headers();
        headers.append("X-MAL-CLIENT-ID", MAL_CLIENT_ID);

        const response = await fetch(url.toString(), {
          method: "GET",
          headers: headers, // Pass the formatted headers object directly
        });
