    ...
    <div className="flex flex-col gap-8 pb-20 animate-in fade-in duration-500">
      ...
      {downloads.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center py-24 gap-3 border-2 border-dashed rounded-3xl">
          <DownloadIcon size={40} className="text-muted-foreground opacity-40" />
          <h2 className="text-xl font-semibold">Nothing saved yet</h2>
          <p className="text-muted-foreground max-w-sm">
            Open an anime and tap "Save for Offline" to keep its info and episode list handy here.
          </p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          {downloads.map((entry, i) => (
            ...
            >
              <Link href={`/anime/${entry.mal_id}`} className="relative aspect-[3/4] overflow-hidden rounded-xl bg-muted shadow-sm block">
                {entry.image ? (
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1.5 opacity-80">
                  {entry.type && <span className="text-xs text-muted-foreground font-mono bg-muted px-1.5 py-0.5 rounded">{entry.type}</span>}
                  {entry.episodes ? <span className="text-xs text-muted-foreground font-mono bg-muted px-1.5 py-0.5 rounded">{entry.episodes} eps</span> : null}
                </div>
              </div>
            </div>
          ))}
