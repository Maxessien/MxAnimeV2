      ...
      {history.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center py-24 gap-3 border-2 border-dashed rounded-3xl">
          ...
          <p className="text-muted-foreground max-w-sm">
            Anime you open will show up here so you can pick up where you left off.
          </p>
          <Link href="/" className="mt-2 text-primary font-medium hover:underline">
            Browse anime
          </Link>
        </div>
      ) : (
        <div className="flex flex-col divide-y divide-border rounded-2xl border bg-card overflow-hidden">
          {history.map((entry, i) => (
            ...
            >
              <Link href={`/anime/${entry.mal_id}`} className="flex-shrink-0 w-14 h-20 rounded-lg overflow-hidden bg-muted">
                {entry.image ? (
              ...
              <Link href={`/anime/${entry.mal_id}`} className="flex-1 min-w-0">
                <p className="font-semibold truncate group-hover:text-primary transition-colors">{entry.title}</p>
                <div className="flex flex-wrap items-center gap-2 mt-1 text-xs text-muted-foreground">
                  {entry.type && <span className="font-mono bg-muted px-1.5 py-0.5 rounded">{entry.type}</span>}
                  {entry.episodes ? <span className="font-mono bg-muted px-1.5 py-0.5 rounded">{entry.episodes} eps</span> : null}
                  <span>
                    Viewed {new Date(entry.viewedAt).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' })}
                  </span>
                </div>
              </Link>
              <button
                onClick={() => remove(entry.mal_id)}
