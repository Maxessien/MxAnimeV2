...
import { AnimeType, TopAnimeFilter } from "@/types/jikan";

export function parseFilter(filter: string): TopAnimeFilter {
  const allow: TopAnimeFilter[] = [
    "airing",
    "upcoming",
    "bypopularity",
    "favorite",
  ...
  return allow.includes(filter as TopAnimeFilter)
    ? (filter as TopAnimeFilter)
    : allow[0];
}

export function parseType(type: string) {
  const allow: AnimeType[] = [
    "tv",
    "movie",
    "ova",
    "special",
  ...
  return allow.includes(type as AnimeType) ? (type as AnimeType) : allow[0];
}
...
export default function TopAnime() {
  ...
  const [filter, setFilter] = useState<TopAnimeFilter>(
    ...
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground text-lg">
          The highest rated and most popular anime of all time.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center bg-card p-4 rounded-xl border shadow-sm">
        ...
        <div className="flex gap-4 w-full sm:w-auto">
          <select
            value={filter}
            onChange={(e) => {
              setFilter(parseFilter(e.target.value));
              setPage(1);
            }}
          ...
          <select
            value={type}
            onChange={(e) => {
              setType(parseType(e.target.value));
              setPage(1);
            }}
