...
export default function Schedule() {
  ...
  const [day, setDay] = useState(searchParams.get('day') || todayKey());
  ...
  return (
    <div className="flex flex-col gap-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground text-lg">See which anime broadcast on each day of the week.</p>
      </div>
      ...
      <div className="flex gap-2 w-full overflow-x-auto pb-2 hide-scrollbar bg-card p-3 rounded-xl border shadow-sm">
        {DAYS.map((d) => (
          <button
            key={d.value}
            onClick={() => { setDay(d.value); setPage(1); }}
            className={`px-5 py-2.5 rounded-lg font-medium text-sm transition-all whitespace-nowrap flex-shrink-0 ${
              day === d.value
          ...
          >
            {d.label}
            {d.value === todayKey() && <span className="ml-1.5 text-[10px] opacity-80 uppercase">Today</span>}
          </button>
        ))}
      </div>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 gap-y-8">
              {entries.map((anime: any, i: number) => (
                <div key={anime.mal_id} className="flex flex-col gap-2 animate-in fade-in zoom-in-95" style={{ animationDelay: `${Math.min(i, 10) * 50}ms`, animationFillMode: 'both' }}>
